          ...
          <div>
            ...
            <div class="bottom-wrapper">
              <div class="button-wrapper">
                <div class="v-button" @click="userAuth()">{{ submitBnt }}</div>
              </div>
            </div>
          </div>
        </div>
...
export default {
  data() {
    return {
      ...
      fileUrl:'http://localhost:8001/api/oss/file/upload',
      userInfo: {
        param: {}
      },
      submitBnt: '提交'
  ...
  methods: {
    ...
    userAuth() {
      ...
      this.submitBnt = '正在提交...'
      userInfoApi.userAuth(this.userAuah).then(response => {
    ...
    getDict() {
      dictApi.findDictByDictCode("CertificatesType").then(response => {
