  ...
  <div class="app-container">
    ...
    <el-table v-loading="listLoading" :data="list"   border fit highlight-current-row>
      ...
          <el-table-column label="操作" width="230" align="center">
          ...
                  <el-button type="danger" v-if="scope.row.status===1" size="mini" @click="updateStatus(scope.row.id,0)">下线</el-button>
                  <el-button type="primary" v-else size="mini"  @click="updateStatus(scope.row.id,1)">上线</el-button>
            </template >
          </el-table-column>
    </el-table>
...
        districtList: [],
  ...
  created() {
      this.fetchData()
      hospApi.findByDictCode('Province').then(
          response=>{
  ...
  methods: {
      updateStatus(id,status){
        hospApi.updateStatus(id,status).then(
          ()=>{
            this.fetchData()
          }
        )
      ...
      fetchData(page=1){
          this.page=page
          hospApi.getHospList(this.page,this.limit,this.searchObj).then(
              response=>{
...
          //this.searchObj.districtCode = null
          hospApi.findByParentId(this.searchObj.provinceCode).then(
              response=>{
                  //console.log(response.data)
